...
def create_custom_color_string_art(jpg_path, svg_path, output_path, num_dots=100, num_lines=1000, resolution=(500, 500),
                                   forbidden_neighbors=0, power=255, line_intensity=255, line_score_method='sum', colors=['#00FFFF', '#FF00FF', '#FFFF00']):
    # Load the image in RGB
    image = Image.open(jpg_path).convert('RGB')
    image = image.resize(resolution)
    image = np.array(image, dtype=np.float64)  # Convert to float64 for easier manipulation
    image /= 255.0  # Normalize to 0-1 range
    ...
    used_ids_dict = {}  # Store used IDs for each color
    color_channels = []

    # Create a channel for each color
    for color_hex in colors:
        color_rgb = np.array(mcolors.hex2color(color_hex))  # Convert hex to RGB (in normalized [0, 1] range)
        # Create a grayscale "channel" image that represents the distance to this color
        channel = 1 - np.mean(np.abs(image - color_rgb), axis=2)
        color_channels.append((channel, color_hex))
        used_ids_dict[color_hex] = []

    def draw_lines(color_channel, color_rgb, color_name):
        # Find the darkest point in the color channel to start the string art
        start_point = np.unravel_index(np.argmax(color_channel, axis=None), color_channel.shape)
        start_idx = np.argmin(np.linalg.norm(dots - start_point[::-1], axis=1))
        ...
        for _ in range(num_lines):
            ...
            # Draw line on canvas for the specific color
            for i in range(3):
                canvas[best_rr, best_cc, i] = np.maximum(0, canvas[best_rr, best_cc, i] - line_intensity / 255.0 * color_rgb[i])
    ...
    # Draw lines for each custom color
    for color_channel, color_hex in color_channels:
        color_rgb = np.array(mcolors.hex2color(color_hex))  # Convert hex to RGB
        draw_lines(color_channel, color_rgb, color_hex)
...
def upload():
    ...
    # Retrieve custom color information
    num_colors = int(request.form.get('numColors', 0))
    colors = request.form.getlist('colors[]')  # Get the list of custom color hex values
    ...
    if svg_file and jpg_file:
        ...
        elif type_art == 'cyanMagentaYellow':
            create_color_string_art(adjusted_img_path, svg_path, result_path, 
                                    num_dots=num_dots, num_lines=num_lines, resolution=resolution, 
                                    forbidden_neighbors=forbidden_neighbors, power=power, 
                                    line_intensity=line_intensity, line_score_method=line_score_method)
        elif type_art == 'customColor':
            # Call the function for creating string art with custom colors
            create_custom_color_string_art(adjusted_img_path, svg_path, result_path, 
                                           num_dots=num_dots, num_lines=num_lines, resolution=resolution, 
                                           forbidden_neighbors=forbidden_neighbors, power=power, 
                                           line_intensity=line_intensity, line_score_method=line_score_method, 
                                           colors=colors)
